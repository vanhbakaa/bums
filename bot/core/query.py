...
class Tapper:
    ...
    async def do_task(self, task, session: cloudscraper.CloudScraper):
        ...
        try:
            ...
            if res.json()["code"] == 0:
                ...
                logger.warning(
                    f"{self.session_name} | Failed to complete {task['name']}: <y>{res.json()['msg']}</y>")
                return None
    ...
    async def upgrade_misc(self, tap_info, session: cloudscraper.CloudScraper):
        try:
            ...
            if settings.AUTO_UPGRADE_JACKPOT_CHANCE and tap_info['bonusChance'][
                'level'] < settings.RECOVERY_MAX_LVL:
                data = {
                    "type": "bonusChance"
                }
                res = session.post(upgrade_misc_api, data=data)
            ...
            while stamina > 0:
                ...
                if resd.json()['code'] == 0:
                    ...
                    for reward in reward_list:
                        logger.success(
                            f"{self.session_name} | <c>[SPIN]</c> | <green>Successfully got <y>{reward['name']}</y>!</green>")
    ...
    async def tap(self, hashCode, seq, collectAmmount, session: cloudscraper.CloudScraper):
        ...
        if res.json()['code'] == 0:
            logger.success(
                f"{self.session_name} | <green>Successfully tapped - Earned <c>{collectAmmount}</c> coin</green>")
        ...
        while True:
            ...
            try:
                    ...
                    if login_data:
                        ...
                        if settings.AUTO_TASK:
                            ...
                            for task in task_list:
                                ...
                                if task['type'] == "fo_mo" or task['taskType'] == "transferTon" or task[
                                    'type'] == "transferTon" or task['type'] == "telegram_share" or task[
                                    'taskType'] == "fo_mo":
                                    continue
                ...
                if self.multi_thread:
                    sleep_ = round(
                        random.uniform(settings.SLEEP_TIME_EACH_ROUND[0], settings.SLEEP_TIME_EACH_ROUND[1]),
                        1)
